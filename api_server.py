#!/usr/bin/env python3
"""
FastAPI Server for Caphè Technologies Workflows
High-performance business automation platform with sub-100ms response times.
"""
...
app = FastAPI(
    title="Caphè Technologies Workflows API",
    description="Professional business automation platform API",
    version="2.0.0"
)
...
class WorkflowSummary(BaseModel):
    ...
    @field_validator('active', 'is_lead_magnet', 'requires_login', mode='before')
    @classmethod
    def convert_bool_fields(cls, v):
        if isinstance(v, int):
            return bool(v)
        return v
...
async def health_check():
    ...
    return {"status": "healthy", "message": "Caphè Technologies Workflows API"}
...
async def search_workflows(
    ...
    tier: str = Query("all", description="Filter by tier (free/pro/starter/business)"),
        ...
        for workflow in workflows:
            try:
                ...
                clean_workflow = {
                    ...
                    # Freemium tier fields
                    'tier': workflow.get('tier', 'pro'),
                    'tier_complexity': workflow.get('tier_complexity', 'intermediate'),
                    'is_lead_magnet': workflow.get('is_lead_magnet', False),
                    'requires_login': workflow.get('requires_login', True)
                }
...
def run_server(host: str = "127.0.0.1", port: int = 8000, reload: bool = False):
    ...
    print(f"☕ Starting Caphè Technologies Workflows API")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Caphè Technologies Workflows API')
